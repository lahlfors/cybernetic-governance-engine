...
class TestRiskAnalystNode:
    ...
    def test_risk_analyst_node_parses_rejection_high_risk(self):
        """'high risk' text triggers REJECTED_REVISE status."""
        with patch.object(adapters, 'run_adk_agent') as mock_run:
            ...
            result = adapters.risk_analyst_node(state)

            assert result["risk_status"] == "REJECTED_REVISE"
            assert "HIGH RISK" in result["risk_feedback"]

    def test_risk_analyst_node_parses_rejection_unsafe(self):
    ...
    def test_risk_analyst_node_parses_rejection_denied(self):
    ...
    def test_risk_analyst_node_parses_approval(self):
...
class TestExecutionAnalystNode:
    ...
    def test_execution_analyst_node_normal_flow(self):
        ...
        with patch.object(adapters, 'run_adk_agent') as mock_run:
            ...
            result = adapters.execution_analyst_node(state)
    ...
    def test_execution_analyst_node_injects_feedback(self):
