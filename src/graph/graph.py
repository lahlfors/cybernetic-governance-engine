...
from .router import risk_router, green_agent_router
...
def create_graph(redis_url="redis://localhost:6379"):
    ...
    # 5. The Strategy -> Risk -> Green Agent -> Execution Loop

    # Execution Analyst (Planner) always goes to Risk
    workflow.add_edge("execution_analyst", "risk_analyst")

    # Risk Analyst routes to either Green Agent (if approved) or back to Planner (if rejected)
    workflow.add_conditional_edges("risk_analyst", risk_router, {
        "execution_analyst": "execution_analyst",
        "green_agent": "green_agent"
    })

    # Green Agent routes to either Trader (if approved) or back to Planner (if rejected)
