...
### 4. Build Process
To generate the `policy.wasm` file, you must run the OPA build command:
```bash
opa build -t wasm -e finance/allow policies/
```
This artifact must be copied into the container at `/app/policy.wasm` (or configured via `OPA_WASM_PATH`).
