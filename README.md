...
3.  **Execution Analyst Agent (Strategy):** Creates detailed execution plans (e.g., VWAP, TWAP).
 4.  **Green Agent (Risk Analyst):** The safety evaluator. Analyzes plans offline to discover new Unsafe Control Actions (UCAs) and updates the governance policy.

### Risk Refinement Loop
The architecture implements a **Self-Correction Loop**. If the **Green Agent** (implemented via the Safety Node in runtime) rejects a plan proposed by the Execution Analyst, the graph automatically routes the feedback back to the Planner with a "CRITICAL" instruction to revise the strategy. This cycle continues until the plan is safe or escalated to a human.
