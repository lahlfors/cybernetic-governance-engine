...
The **Gateway Service** (`src/gateway/server/hybrid_server.py`) acts as the "Central Nervous System" of the Neuro-Cybernetic Bank. It is a dual-mode **HTTP/MCP Service** that mediates all interactions between Agents (the "Brain") and the external world (Markets, Databases, Tools).
...
1.  **Tool Execution & Governance (MCP):**
    *   Exposes tools via **Model Context Protocol (MCP)** over SSE/HTTP.
    *   Receives tool calls from agents via HTTP POST (`/tools/execute`).
    *   Routes them through the **Symbolic Governor**.
    *   Executes the tool only if all checks pass.

2.  **LLM Proxy:**
    *   Proxies requests to vLLM (Reasoning Engine) via `/v1/chat/completions`.
...
Executes a specific capability under strict governance.
