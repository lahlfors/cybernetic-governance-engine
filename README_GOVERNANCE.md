...
We also employ **Systems-Theoretic Process Analysis (STPA)** to identify and mitigate Unsafe Control Actions (UCAs).
...
We use **NeMo Guardrails** running **In-Process** as the first line of defense to ensure the model stays on topic and avoids jailbreaks *before* it even processes a tool call. The in-process architecture is validated in both the main `server.py` entry point and the parallel safety checks.
...
Compute nodes are **stateless** (ephemeral). To maintain session continuity, we use **Redis (Cloud Memorystore)** for session state persistence.
...
*   **Implementation:** `src/gateway/core/structs.py`
*   **Features:**
...
*   **Implementation:** `config/opa/finance_policy.rego`

**State Management (CBF):**
The `ControlBarrierFunction` in `safety.py` supports transactional state updates with `rollback_state(cost)` to handle failed downstream executions.
...
*   **Implementation:** `src/gateway/governance/symbolic_governor.py`
...
*   **Integration:** Embedded in the `SymbolicGovernor`. If the consensus check fails, the trade is blocked even if OPA approves.
*   **Implementation:** `src/governed_financial_advisor/governance/consensus.py`
...
The `SymbolicGovernor` in `src/gateway/governance/symbolic_governor.py` centralizes all governance logic.
...
## 5. Deployment (Sovereign Sidecar)

The architecture is designed for Kubernetes (GKE) with OPA as a sidecar container.
...
For detailed deployment instructions, including the sidecar configuration and startup checks, please see **[README.md](README.md)**.
